...
namespace Panda.Examples.Shooter
{
    public class Unit: MonoBehaviour
    {
        ...
        {
            if( Task.isInspected)
                ThisTask.debugInfo = string.Format("a={0}", ammo);
            return ammo < i;
        }
        ...
        public void WaitArrival()
        {
            ...
            if (!ThisTask.isStarting && navMeshAgent.remainingDistance <= 1e-2)
            {
                ThisTask.Succeed();
                d = 0.0f;
            }

            ThisTask.debugInfo = string.Format("d-{0:0.00}", d );
        }
        ...
        public void AimAt_Target()
        {
            ...
            if( Task.isInspected )
                ThisTask.debugInfo = string.Format("angle={0}", Vector3.Angle(targetDir, this.transform.forward));
